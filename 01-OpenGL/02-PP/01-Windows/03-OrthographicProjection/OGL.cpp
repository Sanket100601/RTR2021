...
int initialize(void) {
	...
	glBindAttribLocation(shaderProgramObject,AMC_ATTRIBUTE_POISITION,"a_position");
	...
	glVertexAttribPointer(AMC_ATTRIBUTE_POISITION,3,GL_FLOAT,GL_FALSE,0,NULL);
	//2n d:paramern :no of row

	glEnableVertexAttribArray(AMC_ATTRIBUTE_POISITION);
...
void display(void) {
	...
	modelViewProjectMatrix = orthographicProjectionMatrix *modelViewMatrix;
