...
#define CHECKARBOARD_WIDTH 64
#define CHECKARBOARD_HEIGHT 64
...
GLubyte checkerBoard[CHECKARBOARD_WIDTH][CHECKARBOARD_HEIGHT][4];;
...
void display(void) {
	...
	translationMatrix = vmath::translate(0.0f,0.0f,-4.0f); //glTranslatef is repleace by this line
...
void loadGlTexture(void) {
	...
	/*texture cha parametr set kar jo integer type cha aahe
		para 2 :tetxure la wrap kar jar texture motha asle tar on x,y depending on  first ,2nd call of
		glTexParameteri and jar texture jar kami padla tar repet kar which is third parameter.
		*/
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, CHECKARBOARD_WIDTH, CHECKARBOARD_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, checkerBoard);

	//glTexEnvf(GL_TEXTURE_ENV,GL_TEXTURE_ENV_MODE,GL_REPLACE);  //texture chya envirnnment cha float parameter set kar
	glBindTexture(GL_TEXTURE_2D, 0);
}
void makeCheckerBoard(void) {
	...
	//code
	//openGL is coloumn major hence we have to move i till checkarBoar_Height but both are
	//same as value here we  make it as CHECKARBOARD_WIDTH
	for (int i = 0; i < CHECKARBOARD_WIDTH; i++) {
		for (int j = 0; j < CHECKARBOARD_HEIGHT; j++) {
