...
GLuint vaoTriangle;
GLuint vboPositionTriangle;
GLuint vboColorTriangle;
...
void initialize(void)
{
	...
	glGenBuffers(1, &vboPositionTriangle);
	glBindBuffer(GL_ARRAY_BUFFER, vboPositionTriangle);
	...
	glGenBuffers(1, &vboColorTriangle);
	glBindBuffer(GL_ARRAY_BUFFER, vboColorTriangle);
...
void display(void)
{
	...
	rotateMatrix    = rotate(angleTriangle, 0.0f, 1.0f, 0.0f);
	...
	// transformations
	translateMatrix = translate(1.5f, 0.0f, -6.0f);
	rotateMatrix    = rotate(angleSquare, 1.0f, 0.0f, 0.0f);
	modelViewMatrix = translateMatrix * rotateMatrix;
...
void uninitialize(void)
{
	...
	{
		glDeleteBuffers(1, &vboPositionTriangle);
		vboPositionTriangle = 0;
	}

	if (vboColorTriangle)
	{
		glDeleteBuffers(1, &vboColorTriangle);
		vboColorTriangle = 0;
	}
