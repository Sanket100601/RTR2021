...
-(int)initialize
{
	...
	glGenBuffers(1, &VBO_pyramid_texcoords);

	glBindBuffer(GL_ARRAY_BUFFER, VBO_pyramid_texcoords);
	...
	if(!texture_kundali)
    {
        fprintf(gpFile, "Error : failed to load kundali.bmp texture.\n");
		[self uninitialize];
        [self release];
        [NSApp terminate:self];
    }
