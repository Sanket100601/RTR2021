...
-(int)initialize
{
	...
	const GLchar* TessellationControlShaderSourcecode =
		...
		"uniform int u_numberOfstripes;" \
		"void main(void)" \
		"{" \
		"	gl_out[gl_InvocationID].gl_Position = gl_in[gl_InvocationID].gl_Position;" \
		"	gl_TessLevelOuter[0] = float(u_numberOfstripes);" \
	...
	if (status == GL_FALSE)
	{
		...
		if (infoLogLen > 0)
		{
			...
			if (log != NULL)
			{
				// Get the compilation log into this allocated buffer.
				GLsizei written;
				glGetShaderInfoLog(TessellationControlShaderObject,
					infoLogLen,
					&written,
					log);
	...
	if (status == GL_FALSE)
	{
		...
		if (infoLogLen > 0)
		{
			...
			if (log != NULL)
			{
				// Get the compilation log into this allocated buffer.
				GLsizei written;
				glGetShaderInfoLog(TessellationEvaluationShaderObject,
					infoLogLen,
					&written,
					log);

				// display the contents of buffer
				fprintf(gpFile, "Tessellation Evalution Shader Compilation Log: %s\n", log);
	...
	numberOfStripsUniform = glGetUniformLocation(shaderProgramObject, "u_numberOfstripes");
	...
	uiNumberOfSegments = 1; // Want to draw 1 Line
	glClearDepth(1.0f);
...
-(void)display
{
	...
	glUniform1i(numberOfSegmentUniform,
		uiNumberOfSegments);
	glUniform1i(numberOfStripsUniform,
		1); // no. strip is 1 line patti

	if (flip)
	{
		uiNumberOfSegments += 1.0;
		if (uiNumberOfSegments >= 30)
		{
			//uiNumberOfSegments = 30;
			flip = false;
			glUniform4fv(lineColorUniform,
				1,
