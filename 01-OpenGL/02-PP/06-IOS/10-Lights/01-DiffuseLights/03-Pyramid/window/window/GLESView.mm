...
- (int)initialize
{
    ...
    const GLchar* vertexShaderSourcecode =
        ...
        "vec3 lightDirection = normalize(vec3(u_lightPosition - iCoordinates));" \
        "diffuse_light_color = u_ld * u_kd * max(dot(lightDirection, transformNormals), 0.0);" \
        "}" \
        "else" \
        "{" \
        ...
        lightingEnableUniform = glGetUniformLocation(shaderProgramObject,
            "u_lightingEnable");
...
- (void)display
{
    ...
    mat4 translationMatrix = mat4::identity();
        ...
        rotationMatrix_y = vmath::rotate(anglePyramid, 0.0f, 1.0f, 0.0f);
