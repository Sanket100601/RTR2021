...
ID3D11Buffer* gpID3D11Buffer_Color_PyramidBuffer = NULL;
...
HRESULT initialise(void)
{
	...
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "Init() in gpID3D11Device::CreateBuffer() for Cube Color Buffer  Failed!!!\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "Init() in gpID3D11Device::CreateBuffer() for Cube Color Buffer  Successed!!!\n");
		fclose(gpFile);
	}
	...
	d3d11BufferDescriptor.Usage = D3D11_USAGE_DEFAULT;
	d3d11BufferDescriptor.ByteWidth = sizeof(float) * _ARRAYSIZE(pyramidColor);
	d3d11BufferDescriptor.BindFlags = D3D11_BIND_VERTEX_BUFFER;

	/* B. Init subresource data structure to put data into buffer */ //glbufferdata
	ZeroMemory((void*)&d3d11SubResourceData, sizeof(D3D11_SUBRESOURCE_DATA));

	d3d11SubResourceData.pSysMem = pyramidColor;


	/* C.Create the Actual Buffer */ // 
	hr = gpID3D11Device->CreateBuffer(&d3d11BufferDescriptor,
		&d3d11SubResourceData,
		&gpID3D11Buffer_Color_PyramidBuffer);

	if (FAILED(hr))
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "Init() in gpID3D11Device::CreateBuffer() for Pyramid Color Buffer  Failed!!!\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "Init() in gpID3D11Device::CreateBuffer() for Pyramid Color Buffer  Successed!!!\n");
		fclose(gpFile);
	}
...
void uninitialise(void)
{
	...
	if (gpID3D11Buffer_Color_PyramidBuffer)
	{
		gpID3D11Buffer_Color_PyramidBuffer->Release();
		gpID3D11Buffer_Color_PyramidBuffer = NULL;
	}
