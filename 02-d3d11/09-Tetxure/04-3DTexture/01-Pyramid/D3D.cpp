...
HRESULT initialise(void)
{
	...
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "Init() in gpID3D11Device::CreateBuffer() for Texcoords Buffer  Failed!!!\n");
		fclose(gpFile);
		return hr;
	}
	else
	{
		fopen_s(&gpFile, "Log.txt", "a+");
		fprintf(gpFile, "Init() in gpID3D11Device::CreateBuffer() for Texcoords Buffer  Successed!!!\n");
		fclose(gpFile);
	}
